...
Meteor.methods({
    ...
    S3upload: function (file,context,callback){
        ...
        knox.putBuffer(buffer, path, headers, function(error){
            if (!error) {
                future.return(path);
            } else {
                //TODO: Throw a correct status code Meteor.Error
    ...
    S3delete: function (path, callback) {
        knox.deleteFile(path, function(error, result) {
            if (error){
                //TODO: Throw a correct status code Meteor.Error
                throw new Meteor.Error(500, "Server error");
            } else if (callback) {
                Meteor.call(callback(result));
            }
        });
    }
});
